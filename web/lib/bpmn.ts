...
/** BPMN 2.0 XML (with DI) from a process, ported from the CLI projection layer. */
...
export function bpmnXml(model: Model): string | null {
  const process = model.processes[0];
  if (!process || !process.tasks.length) return null;
  ...
  const outgoing = (id: string) =>
    flows.filter((f) => f.from === id).map((f) => `      <bpmn:outgoing>${f.id}</bpmn:outgoing>`).join("\n");
  const incoming = (id: string) =>
    flows.filter((f) => f.to === id).map((f) => `      <bpmn:incoming>${f.id}</bpmn:incoming>`).join("\n");

  const flowEls: string[] = [];
  flowEls.push(`    <bpmn:startEvent id="${startId}" name="Start">\n${outgoing(startId)}\n    </bpmn:startEvent>`);
  tasks.forEach((t, i) => {
    const id = taskIds[i]!;
    flowEls.push(`    <bpmn:task id="${id}" name="${esc(t.name)}">\n${incoming(id)}\n${outgoing(id)}\n    </bpmn:task>`);
  });
  flowEls.push(`    <bpmn:endEvent id="${endId}" name="End">\n${incoming(endId)}\n    </bpmn:endEvent>`);
  for (const f of flows) flowEls.push(`    <bpmn:sequenceFlow id="${f.id}" sourceRef="${f.from}" targetRef="${f.to}" />`);
...
<bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL"
                  ...
                  id="Definitions_archmantic" targetNamespace="http://archmantic/bpmn">
